...
parser = ArgumentParser(
    description='Utility script to load daily data into Oracle Database')
parser.add_argument(
    "-l",
    "--log",
...
db_default_address = 'system/1234@localhost:1521/xe'
db_address = db_default_address if not args.database else args.database
conn = ora.connect(db_address)
cursor = conn.cursor()

# insert into the job table
queryjob = """insert into JOBS_SCHEDULE (id, DESCRIPTIONS,START_DATE,STATUS)
values (JOB_SEQ.NEXTVAL,:bdesc,sysdate,:bstatus_flag)"""
...
try:
    ...
    logging.write("################### START ##################\n")
    logging.write("############################################\n")
    path = "../../data/" if not args.data else args.data
    ...
    for f in files:
        ...
        files_arx = re.sub(
            '[^0-9a-zA-Z._,]+',
            '',
            str(cursor.fetchall()))[:-1].split(',,')
        if f.replace(' ', '') not in files_arx:
            # read the csv files
            nobel_data = pd.read_csv(path+f, names=(
                "Agent Hours",
                "col1", "col2",
                "col3", "col4",
                "col5", "col6",
            ...
            header_dupes = nobel_data[nobel_data['Agent Hours']
                                      == 'Agent Name'].index.tolist()
            ...
            index_list = nobel_data[nobel_data['Agent Name'] ==
                                    'Agent Name'].index.tolist()
            nobel_data = nobel_data.drop(index=index_list)
            nobel_data = nobel_data.reset_index(drop=True)

            # query3
            querystring3 = """insert into STAGING_NOBLE (AGENT_NAME,CODE,
            CONTACT_DATE,LOGON_TIME,LOGOFF_TIME,CONNECTED,WAITING,PAUSED,
            DEASSIGN,ACW,TOTAL,FILE_NAME) VALUES
            ...
            for i in range(len(nobel_data)):
                ...
                status_flag = "Running"
                queryjob = """UPDATE JOBS_SCHEDULE SET
                status = :bstatus_flag WHERE id = :brun_id"""
                cursor.execute(
                    queryjob, bstatus_flag=status_flag, brun_id=run_id)
                conn.commit()
            ...
            conn.commit()
            logging.write("Total"+str(len(nobel_data)) +
                          'rows were inserted to the table STAGING_NOBLE\n')
            logging.write("             \n")
...
# update the job table
queryjob = """UPDATE JOBS_SCHEDULE SET status = :bstatus_flag,
end_date = sysdate WHERE id = :brun_id """
